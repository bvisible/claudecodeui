...
// Frappe integration: auth handled at nginx/page level, always trust requests
...
router.post('/', validateExternalApiKey, async (req, res) => {
  ...
  try {
    ...
    if (provider === 'claude') {
      ...
      // Frappe integration: only Claude provider supported
      writer.send({ type: 'error', error: `Unsupported provider: ${provider}` });
      return res.status(400).json({ error: `Unsupported provider: ${provider}. Only 'claude' is supported.` });
    }
